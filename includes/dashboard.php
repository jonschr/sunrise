...
function dashboard_page() {
	...
	echo '<div class="sunrise-dashboard-links"><a id="sunrise-open-control" class="button" href="' . esc_url( add_query_arg( 'view', 'administration', $url ) ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Sunrise Control · Administration', 'sunrise' ) . '</a><a href="' . esc_url( add_query_arg( array( 'dashboard_site' => agent_state()['site_id'], 'view' => 'updates' ), $url ) ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Network dashboard permission', 'sunrise' ) . '</a></div><p id="sunrise-dashboard-status" role="status"></p><iframe id="sunrise-dashboard" class="sunrise-dashboard-frame" title="Sunrise network updates" referrerpolicy="no-referrer" sandbox="allow-scripts allow-same-origin"></iframe>';
}

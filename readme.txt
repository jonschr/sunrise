...
= 0.3.10 =
* Replace the embedded Control dashboard with local site connection settings and direct check-in and re-authentication controls.
* Add a copyable 14-day diagnostic log for cron, transport, identity, and reconnection failures without storing credentials or request bodies.
* Retire the unfinished migration workbench and its transfer protocol.

= 0.3.9 =
* Recover same-site moves through Control on ordinary traffic when cron is unavailable.
* Publish immutable update metadata with each GitHub release.

= 0.3.8 =
* Recover an approved reconnection when its locally saved enrollment ID was lost during migration or concurrent updates.

= 0.3.7 =
* Finish an approved reconnection after migration cleanup changes local identity files or administrator passwords.

= 0.3.6 =
* Exchange approved reconnections during WordPress initialization, without relying on cron or PHP shutdown callbacks.
...
= 0.3.2 =
* Restore missing check-in events during ordinary WordPress initialization and reserve the next event before synchronization begins.

= 0.3.1 =
* Show the installed Sunrise Client version beneath the Sunrise Control version in the WordPress dashboard.

= 0.3.0 =
* Add persistent two-admin site pairing and direct push/pull database migrations with automatic serialized-safe URL/path replacement and destination recovery tables.

= 0.2.30 =
* Ignore unfamiliar future license metadata safely while retaining strict input validation.
...
= 0.2.27 =
* Move Administration and Site settings from the dashboard header into the Sunrise sidebar.

= 0.2.26 =
* Retry a configured premium plugin key once after each new update failure, even when an earlier activation attempt rejected the same saved key.

...
Sunrise has Network and Migrations pages at the bottom of the WordPress administrator menu. Network operations remain scoped to the current administrator's Control connection. A saved migration pair is installation-owned and usable by any current administrator, but creating it requires separate administrator approval on both selected connected sites. Open Sunrise Control selects the current connection's network using a navigation hint; Control independently checks the human's membership. No site credential or human token travels in this URL.
...
## Direct paired database migrations (0.3.0)

The WordPress Migrations page can push or pull a complete single-site WordPress database directly between two saved site pairs. Pairing starts from one site, requires a current administrator to approve on the other site, and stores a separate encrypted 256-bit secret on each installation. A pair authorizes only those two installation identities; selecting another site in the same Control network grants nothing. Multiple pairings may coexist. Control supplies the connected-site directory but does not relay database rows.

Every direct request is HTTPS except for an explicitly local initiator, signed over its method, REST route, timestamp, nonce, and body, and rejected on replay. Live-to-local requests cannot be initiated by the live site. Transfers use bounded hash-verified chunks and private non-public workspaces. Tables are validated and staged under generated names before one atomic `RENAME TABLE` publishes the database. Source-corresponding destination tables are retained under generated backup names; destination-only tables remain in place. The destination screen can atomically restore the retained originals.

Users migrate with the database. Sunrise automatically maps the source `siteurl`, `home`, and normalized WordPress root to their destination values in text, JSON, and ordinary PHP-serialized strings without calling PHP `unserialize`; string byte lengths are rebuilt. Unsupported custom serialization stops staging instead of risking corruption. Destination installation identity, pair secrets, cron, and Sunrise-owned options are retained. If copied users invalidate a Control credential owner, that enrollment is removed and the current administrator is asked to reconnect; the migration pair remains usable.

The initial safety ceiling is 200 InnoDB tables, 512 MiB of row data, 4 MiB per row, and 20 seconds for the consistent snapshot and each staging table. Database runs are separate from the existing settings/file workflow until files also use the direct pair transport. No production-host certification or live migration has been performed for this version.

## Legacy read-only transfer groundwork

The legacy inventory endpoint can inspect this installation without making a remote request. `GET /sunrise/v1/transfers/inventory?scope=posts|media|options|plugins|themes|tables&after=...` requires the current administrator's valid Control connection and pages at 50 records. Post/media records include a length-delimited digest of selected raw post fields (up to 2 MiB); metadata, terms, comments, linked records and file bytes are excluded. Larger records return no fingerprint. Component and table listings are metadata-only; direct database execution uses the paired workflow above. Pagination is live, not a transactionally frozen snapshot.
...
The pure `transfer_rewrite` helper supports explicit text, JSON and serialized formats. It caps each value at 2 MiB, serialization traversal at 10,000 tokens and depth 32, and mapping count at eight. It uses a small wire parser, never PHP `unserialize` on transferred input, and safely preserves ordinary objects and references. Custom serialization, unsupported tags, duplicate serialized keys, trailing bytes, wrong lengths, runtime integer overflow and URL-bearing keys are rejected. Nested serialization retains byte lengths. JSON string rewriting preserves numeric bytes and surrounding structure. URL replacements prefer the longest known base, respect path/host boundaries and do not cascade into another mapping. JSON slash escapes are handled; arbitrary URL encodings, opaque signed/checksummed payloads, embedded binary formats and unknown plugin reference semantics require their own handlers.

Checks: `php tests/transfer-rewrite-check.php` covers bounded URL/path rewriting without WordPress or a database. `wp --user=1 eval-file tests/transfer-inventory-check.php` covers the legacy inventory path on disposable fixtures. Direct pairing and database state have pure checks; an actual two-site migration remains intentionally unrun for user testing.


## Legacy Control-relayed settings and file preparation

Network administrators prepare transfers from the WordPress Migrations page. The current site is always one endpoint: choose one other connected site, then choose whether to push this site to it or pull it into this site. The plugin delegates creation to `POST N/transfers` with a UUID `Idempotency-Key`; the service keeps the selection immutable, verifies both connections belong to the approved network, and rejects equal installation URLs. At most five pending previews per endpoint may exist. Preparation expires after 72 hours. This per-operation workflow does not turn update permissions or site enrollment into standing migration authority.
...
WordPress renders the exact destination before/after values using text nodes and delegates `POST N/transfers/{id}/approve` with `{side,revision,plan_hash}`. Source and destination approvals are bound to the same immutable canonical plan hash and a shared one-hour expiry. The existing network-dashboard grant supplies the administrator authority; the site credential alone cannot approve work. `POST N/transfers/{id}/cancel` takes `{revision}`. Stale edits fail with 412; changed hashes or expired previews cannot be approved. Site generation changes/disconnection and requester/approver access removal cancel the request; reactivation does not revive it. The destination runner validates current values, commits selected changes with a local backup journal, and reports the retained outcome. Unknown outcomes stay fenced for inspection; retries never repeat a write. Content writes remain disabled here; database tables use the direct paired workflow.
...
Checks: `npm test` includes scoped preparation/approval, duplicate delivery, foreign-site denial, direct SQL mutation guards, expiry, revocation, queue limits and failed preparation. `tests/migration-workbench-check.php` checks WordPress-owned creation/approval and strict request validation. `tests/agent-transfer-check.php` checks shared locking, persistent exact retries, cancellation and older-service negotiation. `node --experimental-strip-types tests/cove-transfer-work-check.mjs` exercises both outbound Cove agents and verifies unchanged settings/identity. All temporary request fixtures are removed; transport sequences are never rewound.

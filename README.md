...
The central-service agent pushes inventory outbound about every five minutes through WP-Cron and falls back to ordinary WordPress traffic when a host disables traffic-driven cron. The local Site settings page shows the last successful check-in, configured cadence, and next cron event; **Check in now** runs synchronously without cron. It also supports approved credential re-authentication and a copyable, bounded diagnostic log. CLI sync and authenticated wake requests remain available. A new connection reports its current update inventory immediately after approval while the initiating WordPress tab is open. Newly delivered policies are acknowledged in the same synchronization. Local/private sites can report without accepting inbound requests. See [Sunrise Control README](</Users/jonschroeder/Local Sites/Sunrise Control/README.md>) for the local pilot configuration and its limitations.
...
Sunrise has a Site settings page at the bottom of the WordPress administrator menu. Open Sunrise Control selects the current connection's network using a navigation hint; Control independently checks the human's membership. No site credential or human token travels in this URL.

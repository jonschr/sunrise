...
try {
 $state = Sunrise\agent_state();$state['errors_enabled'] = false;Sunrise\agent_store( $state );
 ...
 Sunrise\capture_error_summary( $error );sunrise_errors_assert( ! Sunrise\error_summaries(), 'Capture is opt-in' );
 ...
 for ( $i = 0; $i < 55; $i++ ) { delete_option( 'sunrise_error_capture_lock' );Sunrise\capture_error_summary( array_merge( $error, array( 'line' => $i + 100 ) ) ); }
 sunrise_errors_assert( 50 === count( Sunrise\error_summaries() ), 'The local group cap holds during a storm' );
